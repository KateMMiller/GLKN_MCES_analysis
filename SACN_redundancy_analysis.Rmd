...
leaflet(height = 650, width = 900) |> 
    ...
    addAwesomeMarkers(data = samp_locs, ~Longitude, ~Latitude, 
                      ...
                      labelOptions = labelOptions(noHide = T, textsize = "12px", textOnly = T, direction = 'auto',
                                                  style = list("color" = "navy")),# offset = c(-10,-5)),
                      # popup on click
...
psy_plot <- 
  ggplot(params_site_year1, aes(x = Site, y = Year)) +
    ...
    waterGLKN::theme_WQ() +
    labs(x = NULL) +
    scale_y_continuous(breaks = c(2006, 2010, 2014, 2020, 2024))
